# Mission:

 Summit the password of the current level to a port on localhost in the range 31000 -> 32000.

 First find out which of these ports have a server listening on them. Then find out which of those speak SSL/TLS and which don’t. There is only 1 server that will give the next credentials, the others will simply send back to you whatever you send to it.

# Steps:

## Nmap
 First, to scan which ports on localhost have a server listening on them, i use `nmap` command.

 ![Nmap](Nmap.png)

 For details, i want to scan in range, so i will use
 ```bash
  nmap -p port1 - port2 host
 ```

 ![Ports](Ports.png)

## Openssl
 As i see, i have 5 ports which is in open state.
 
 Now i want to know which port is speak SSL/TLS, `openssl s_client` command will help me solving this, i will scan every single port.


 ![Keyupdate](KeyUpdate.png)

 When i scan on port 31518, after enter the password in bandit15, it print out `KEYUPDATE`, after searching `KEYUPDATE` on google, i find out that to turn off the interactive commands using the `-ign_eof` option.

 ![Port31518](Port31518.png)

 After enter the password again, i received the password which i enter, as discriptions say:

 `There is only 1 server that will give the next credentials, the others will simply send back to you whatever you send to it.`

 So 31518 is not what i want to find, continue scanning port 31691,31790.

 ![Goal](Goal.png)

 ## In port 31790, i receive sshprivate key which is the password for bandit17.

 the password:
 
 `-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
`